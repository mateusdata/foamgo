...
export default function AvatarCompany() {
    ...
    const handleUploadImage = async (asset: ImagePicker.ImagePickerAsset) => {
        try {
            ...
            console.log('[AvatarCompany] upload failed', {
    ...
    const removeImage = async () => {
        try {
            setLoading(true);
            await api.delete('/companies/avatar');
            await refreshUser();
    ...
    const confirmRemoveImage = () => {
        if (!user?.company?.avatar) {
            Alert.alert('Atenção', 'Você não possui foto do lava jato.');
            return;
        }
    ...
    const renderAvatar = () => {
        if (user?.company?.avatar) {
            return <Avatar.Image size={130} source={{ uri: user.company.avatar }} />;
        }
    ...
    return (
        <View style={styles.container}>
            <Pressable style={styles.avatarContainer}>
                ...
                <ThemedText style={styles.userName}>{user?.company?.name || 'Lava Jato'}</ThemedText>
            </Pressable>
